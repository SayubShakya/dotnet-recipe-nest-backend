...
public class RecipeController : BaseController
{
    ...
    {
        var serverResponse = new ServerResponse(recipeService.GetAll(), "Recipe list!", 200);
        return ToJsonResponse(serverResponse);
    }

    public string GetById(int id)
    {
        var serverResponse = new ServerResponse(recipeService.GetById(id), "Recipe found!", 200);
        return ToJsonResponse(serverResponse);
    }
    ...
    {
        var success = recipeService.Save(request);
    ...
    {
        var success = recipeService.Update(request);
    ...
    {
        var serverRespose = new ServerResponse(recipeService.DeleteById(id), "Recipe deleted!", 200);
